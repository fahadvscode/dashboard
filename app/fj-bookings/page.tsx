...
export default function FJBookings() {
  ...
  async function fetchBookings() {
    ...
    try {
      ...
      const query = applyAppointmentDateFilter(
        supabase.from('fj_bookings').select('*'),
        range
  ...
  const exportToCSV = () => {
    ...
    a.href = url
    a.download = `fj-bookings-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  }
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">FJ Bookings</h1>
