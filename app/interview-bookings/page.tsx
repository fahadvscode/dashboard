...
function mapRowToBooking(row: Record<string, unknown>): Booking {
  ...
  return {
    ...
    appointment_date: String(copy.appointment_date || ''),
    appointment_time: String(copy.appointment_time || ''),
    slot_start: (row.slot_start as string) || null,
    slot_end: (row.slot_end as string) || null,
    application_id: (row.application_id as string) || null,
    appointment_type: 'In-Person Interview',
...
export default function InterviewBookings() {
  ...
  async function fetchBookings() {
    try {
      ...
      setBookings((data || []).map((row) => mapRowToBooking(row as Record<string, unknown>)))
    } catch (error) {
  ...
  const exportToCSV = () => {
    ...
    a.href = url
    a.download = `interview-bookings-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-2">{bookings.length} total interviews</p>
        </div>
          ...
          <div className="relative w-full max-w-2xl max-h-[90vh] rounded-2xl bg-white shadow-2xl overflow-y-auto">
            ...
            <div className="grid gap-4 px-6 py-4 md:grid-cols-2">
              <div className="space-y-3 text-sm">
                ...
                {selectedBooking.application_id && (
                  <div className="flex items-center text-gray-700">
                    <Tag className="mr-3 h-4 w-4 text-gray-400" />
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-500 mr-3 w-24">Application:</span>
                    <span>{selectedBooking.application_id}</span>
                  </div>
                )}
                {selectedBooking.slot_start && (
                  <div className="flex items-center text-gray-700">
                    <Clock className="mr-3 h-4 w-4 text-gray-400" />
                    <span className="text-xs font-semibold uppercase tracking-wide text-gray-500 mr-3 w-24">Slot start:</span>
                    <span className="text-sm">{selectedBooking.slot_start}</span>
                  </div>
                )}
              </div>
