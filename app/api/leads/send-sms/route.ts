...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.error('send-sms: missing Supabase URL or key')
    ...
    if (!accountSid || !authToken || !twilioPhoneNumber) {
      console.error('send-sms: missing TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, or TWILIO_PHONE_NUMBER')
    ...
    const { to, message, leadName, leadId, leadTable } = body
